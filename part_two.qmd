...
Pakatan Harapan and Pakatan Rakyat fielded both more Malay candidates proportionally than Barsian Nasional. Of the major coalitions, Perikatan Nasional had the lowest proportion of minority candidates. 
...
Whilst many coalitions might be multiethnic in nature, [Mustafa, Kartini and Nazri](https://www.frontiersin.org/journals/political-science/articles/10.3389/fpos.2026.1721966/full) argue that, at least under the period of BN dominance, coalitions were not consociational, but were "sustained more by UMNO’s control and patronage than by genuine power-sharing". 
...
However, though their share of votes is lower than it has ever been, UMNO still, marginally, received more votes than any other party. 
...
This speaks to not only UMNO, PAS and Bersatu have pushed, resulting in 65% of respondents in Muslim Youth Survey saying that Muslims should only vote for Muslim leaders, but also to an unfair economic system where whatever economic development is meagre and piecemeal, and insufficient to attract voters.
...
## So what does this mean for Pakatan Harapan? 
...
Whilst public support for BN has faded considerably, between BN and PN, there was majority support for a Malay-supremacist (*Ketuanan Melayu*) government which collected more than 50% of votes in GE15. But BN pulled out of the coalition 
...
Can Harapan/Rakyat win 
the testing is the 


Poverty incidence, population density, proportion_bumi_peninsula, proportion_bumi_east_malaysia, proportion_chinese, proportion_indian, gini, total_population
...
```{r}
glmnet_model <- train(
  pc_ph_votes ~ population_density + 
    pc_bumi_peninsula + pc_bumi_east_malaysia + pc_indian + 
    gini, 
  data = training, 
  method = "glmnet", 
  metric = "RMSE", 
...
test_plot <- testing |> 
  ...
  labs(fill = "Winning\ncoalition", 
       x = "% PH votes in GE-15", 
       y = "Projected % of PH votes")

ggplotly(test_plot, tooltip = c("text"))
...
ballots |> filter(date == "2021-12-18") |> count(state, seat)
