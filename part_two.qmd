...
sqrt(mean(testing$pc_ph_votes - testing$step_pred)^2)

sqrt(mean(testing$pc_ph_votes - testing$glmnet_pred)^2)

stepwise_model |> summary()

rss_step <- sum((testing$pc_ph_votes - testing$step_pred)^2)
tss <- sum((testing$pc_ph_votes - mean(testing$pc_ph_votes))^2)
1 - rss_step / tss

rss_glm <- sum((testing$pc_ph_votes - testing$glmnet_pred)^2)
1 - rss_glm / tss
```
...
safe_seats <- testing |> 
  ...
  mutate(
    ph_win_chance = ifelse(proj_ph_vote > .5, 1, proj_ph_vote * modifier * 100)) |> 
...
We may improve these predictions if we had access to additional small-area demographic data, such as education levels as well as a breakdown of the population by religion. 
...
However, the margin by which PH/PR has won has been trending downwards. And its footprint is narrowing, especially with PKR focusing on [safe seats](https://www.freemalaysiatoday.com/category/nation/2026/04/16/avoiding-weak-seats-a-realistic-strategy-says-analyst). It seems that they too are predicting losses for PH and are seeking 
...
## Barisan Nasional vs. Perikatan Nasional
...
pn_bn_test <- pn_bn_test |> 
  cbind(preds_pn_bn)

```

Let's use the same combination of stepwise linear modelling and glmnet to examine to see how well we can predict 
...
BN is expected to lose even more ground to PN in the upcoming 16th general election, making further inroads into Johor, Pahang and Perak. It will largely only be BN's competitiveness in East Malaysia that will keep it afloat. 

In the plot below, negative values on the y-axis indicate PN winning more votes than BN. We note that the model predicts PN winning more votes than BN in almost all the constituencies than BN won in GE-15. 
...
#| out-width: 100%
#| out-height: 100%
pred_pn_bn_coalition <- pn_bn_test |> 
  left_join(
    ballots |> 
      filter(election == "GE-15" & result %in% c("won", "won_uncontested")) |> 
      select(state, seat, coalition), 
