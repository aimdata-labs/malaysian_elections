...
```{r}
news_raw <- read_csv("./raw_data/news_v1.csv")
...
since there isn't the full text of each article

```{r}
news_raw |> count(section, category)

news <- news_raw |> 
  filter(section %in% c("Metro", "News", "Opinion", "Starpicks") & 
           category %out% c("A Winning Factor", 
                            "At Full Swing", 
                            "Bangkok Buzz",
                            "Sports Box", 
                            "Sports Guru", 
