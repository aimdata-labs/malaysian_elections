...
In the plot below, negative values on the y-axis indicate PN winning more votes than BN. We note that the model predicts PN winning more votes than BN in almost all the constituencies that BN won in GE-15. 
...
Perikatan Nasional is predicted to be be largest coalition in parliament in 2016. Using the same combination of models as before, we predict that PN will win **`r pn_safe_seats`** out of the 222 seats in parliament, up from `r pn_ge_15` in GE-15, making further inroads into Selangor, Johor and Melaka. 
...
We predict that UMNO (BN) will once again play kingmaker. But depending on the scale of their losses, it is possible that their only choice will be Perikatan Nasional as they might not have sufficient seats to from a government with Pakatan Harapan if PH wins only `r safe_seats` as predicted. 

If we take a look at the most marginal seats, BN's largest competitors are PH and PN. If support for BN to further erode, as we predict, PH would no longer be sure to win in three-cornered fights (where it has picked up most of its post-2018 wins), especially if Malay voters are consolidated behind PN. 
