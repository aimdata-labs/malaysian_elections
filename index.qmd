...
# Sarawak makes this all one giant mess
gdp_district |> 
  filter(sector == "all" & date == "2020-01-01") |>
  mutate(economic_district = district) |> 
  
  mutate(district = case_when(
    economic_district == "Tangkak" & state == "Johor" ~ "Ledang", 
    economic_district == "Kulim" & state == "Kedah" ~ "Kulim-Bandar Baharu", 
    economic_district == "Kota Setar" & state == "Kedah" ~ "Alor Setar",
    economic_district == "Kuala Muda" & state == "Kedah" ~ "Sungai Petani", 
    economic_district == "Kulim" & state == "Kedah" ~ "Kulim-Bandar Baharu", 
    economic_district == "Yan" & state == "Kedah" ~ "Jerai", 
    economic_district == "Kecil Lojing" ~ "Gua Musang", 
    economic_district %in% c("Asajaya", "Samarahan") ~ "Kota Samarahan", 
    economic_district %in% c("Bau", "Lundu") ~ "Sepanggar", 
    TRUE ~ economic_district
  )) |> 
  # This is for all the small districts in Sarawak
  # Maybe leftjoin the district back in
  group_by(economic_district, )
  select(economic_district, district, state) |> 
  right_join(
    census_fed |> 
      mutate(district = str_sub(parlimen, start = 7L)) |> 
      distinct(state, district, parlimen), 
  ...
  mutate(economic_district = case_when(
    state == "Perlis" ~ "Perlis", 
    district == "Jerlun" ~ "Kubang Pasu", 
    district == "Kuala Kedah" ~ "Kota Setar", 
    district == "Merbok" ~ "Kuala Muda", 
    district == "Padang Serai" ~ "Kulim", 
    district == "Pengkalan Chepa" ~ "Kota Bharu", 
    district == "Rantau Panjang" ~ "Pasir Mas",
    ...
    district == "Balik Pulau" & state == "Pulau Pinang" ~ "Barat Daya",
    ...
    district == "Hulu Selangor" & state == "Selangor" ~ "Ulu Selangor",
    district == "Hulu Langat" & state == "Selangor" ~ "Ulu Langat",
    district == "Tanjong Karang" & state == "Selangor" ~ "Kuala Selangor",
    ...
    district == "Hang Tuah Jaya" & state == "Melaka" ~ "Melaka Tengah",
    ...
    district == "Iskandar Puteri" & state == "Johor" ~ "Johor Bahru",
  ...
  filter(is.na(economic_district))
```

```{r}
census_dun |> 
  filter(str_detect(dun, "Kota Setar"))

#Sarawak is so messy, I don't want to do it now
gdp_district |>
  filter(state == "Sarawak" & sector == "all" & date == "2020-01-01" & series == "growth_yoy")
...
Recalling our criticism of Anwar in the [addendum on malapportionment](https://aimdata-labs.github.io/malaysian_elections/), is his bid with GRS to increase the number of seats in Sabah meant to engineer a more lasting parliamentary majority without needing to win the popular vote? We would emphasise that any further malapportionment would be, ultimately, an anti-democratic exercise. 
...
Let's now predict Pakatan Harapan's performance in the upcoming general election (in this case, the percentage of votes we anticipate them to receive). 
...
Below, the performance of PH in GE-15 is plotted against their predicted performance in GE-16, with each point being a federal electoral constituency. The combined stepwise-glmnet model predicts that PH's vote share will shrink in GE-16. 
...
Perikatan Nasional are anticipated to win **`r pn_safe_seats`** seats, whilst Pakatan Harapan are anticipated to only win **`r safe_seats`** seats. However, there is only a very small difference in PH's and PN's shares of the national vote. Though, to have a chance at being the largest coalition, Pakatan must win all of Selangor and extend its reach into Johor and Pahang -- an unlikely prospect. 
...
It might not be possible to re-form the Madani government. This is dependent on the scale of BN's losses and the scale of PN's victory. We were not able to develop a model that accurately predicted BN's district-level vote share using demographic data, probably due to BN's performance being largely determined by non-demographic factors (i.e. running a particularly strong candidate in Titiwangsa or the strengthening of unfavourable perceptions of the coalition). It is possible that they will not even be in a position to play kingmaker, especially if our predictions about their poor performance against Perikatan hold true. 
...
The convergence of the two social cleavages of race and religion, as well as UMNO's downfall, have made it probable that Perikatan Nasional will be the largest coalition in parliament. 
...
However, we should note than the r-squared is quite low, when compared to the earlier two models. BN's performance is actually quite difficult to model from demographic data. 
...
We could very conceivably improve these predictions if we had access to additional small-area demographic and economic data, such as education levels, small-area GDP data as well as a breakdown of the population by religion. 
