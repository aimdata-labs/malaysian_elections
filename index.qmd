...
ggsave("./plots/parliamentary_misapportionment.png", height = 7, width = 11, units = "in")
```
...
The voting power of Malays (alongside other races) has been severely compromised in numerous electoral constituencies; especially in Bangi, Kota Raja, Subang, Tebrau, Damansara, Iskandar Puteri and Shah Alam. Each of these has more than 400,000 residents being represented by only one MP. 

Additionally, the vote shares of minorities in Peninsula Malaysia are particularly compromised, with their vote shares being roughly 25% less than what their population should command. Minority votes are worth less than Bumiputera votes in both the peninsula and in East Malaysia. 
...
census_dun |> 
  ...
  labs(title = "Malapportionment is a problem at the state level as well",
       subtitle = "State labels include how much larger the average constituency is from the smallest constituency\nResults from most recent election", 
       x = "Population", 
       y = "", 
       fill = "Coalition")
...
That such severe malapportionment persists even in states under Pakatan Harapan control (which should be the coalition most disposed towards redelineation) speaks towards the priorities of the current government and a disregard for the fair representation of its constituents. 
